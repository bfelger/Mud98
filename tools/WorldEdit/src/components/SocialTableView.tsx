...
export function SocialTableView({
  rows,
  columns,
  defaultColDef,
  toolbar,
      ...
      {rows.length ? (
        <div className="ag-theme-quartz worldedit-grid">
          <AgGridReact
            ...
            onRowClicked={(event) => onSelectSocial(event.data?.index ?? null)}
            onGridReady={(event) => {
        ...
        <div className="entity-table__empty">
          <h3>Socials will appear here</h3>
          <p>Load a socials file from the data directory to get started.</p>
        </div>
      )}
    </div>
  );

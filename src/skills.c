...
void do_gain(CharData* ch, char* argument)
{
    ...
    if (!str_prefix(arg, "list")) {
        ...
        for (gn = 0; gn < max_group; gn++) {
            ...
            if (!ch->pcdata->group_known[gn]
                && group_table[gn].rating[arch] > 0) {
                sprintf(buf, "%-18s %-5d ", group_table[gn].name,
                        group_table[gn].rating[arch]);
                send_to_char(buf, ch);
    ...
    if (gn > 0) {
        ...
        if (group_table[gn].rating[arch] <= 0) {
            act("$N tells you 'That group is beyond your powers.'", ch, NULL,
                trainer, TO_CHAR);
            return;
        }

        if (ch->train < group_table[gn].rating[arch]) {
        ...
        act("$N trains you in the art of $t", ch, group_table[gn].name, trainer,
            TO_CHAR);
        ch->train -= (int16_t)group_table[gn].rating[arch];
        return;
    }
    ...
    if (sn > -1) {
        ...
        if (skill_table[sn].rating[arch] <= 0) {
            act("$N tells you 'That skill is beyond your powers.'", ch, NULL,
                trainer, TO_CHAR);
            return;
        }

        if (ch->train < skill_table[sn].rating[arch]) {
        ...
        act("$N trains you in the art of $t", ch, skill_table[sn].name, trainer,
            TO_CHAR);
        ch->train -= (int16_t)skill_table[sn].rating[arch];
        return;
    }
...
void do_spells(CharData* ch, char* argument)
{
    ...
    for (sn = 0; sn < max_skill; sn++) {
        ...
        if ((level = skill_table[sn].skill_level[arch]) < LEVEL_HERO + 1
            ...
            found = true;
            level = skill_table[sn].skill_level[arch];
...
int exp_per_level(CharData* ch, int points)
{
    ...
    if (points < 40)
        return 1000 * (race_table[ch->race].arch_mult[arch] ?
            race_table[ch->race].arch_mult[arch] / 100 : 1);
    ...
    return expl * race_table[ch->race].arch_mult[arch] / 100;
}
...
bool parse_gen_groups(CharData* ch, char* argument)
{
    ...
    if (!str_prefix(arg, "add")) {
        ...
        if (gn != -1) {
            ...
            if (group_table[gn].rating[arch] < 1) {
                send_to_char("That group is not available.\n\r", ch);
                return true;
            }

            /* Close security hole */
            if (ch->gen_data->points_chosen + group_table[gn].rating[arch]
                > 300) {
                send_to_char(
            ...
            ch->gen_data->points_chosen += group_table[gn].rating[arch];
            gn_add(ch, gn);
            ch->pcdata->points += (int16_t)group_table[gn].rating[arch];
            return true;
        }
        ...
        if (sn != -1) {
            ...
            /* Close security hole */
            if (ch->gen_data->points_chosen + skill_table[sn].rating[arch]
                > 300) {
                send_to_char(
                    "You cannot take more than 300 creation points.\n\r", ch);
                return true;
            ...
            ch->gen_data->points_chosen += skill_table[sn].rating[arch];
            ch->pcdata->learned[sn] = 1;
            ch->pcdata->points += (int16_t)skill_table[sn].rating[arch];
            return true;
        }
...
void check_improve(CharData* ch, SKNUM sn, bool success, int multiplier)
{
    ...
    chance /= (multiplier * skill_table[sn].rating[arch] * 4);
    chance += ch->level;
    ...
    if (success) {
        ...
        if (number_percent() < chance) {
            ...
            send_to_char(buf, ch);
            ch->pcdata->learned[sn]++;
            gain_exp(ch, 2 * skill_table[sn].rating[arch]);
        }
    }
...
void group_add(CharData* ch, const char* name, bool deduct)
{
    ...
    if (sn != -1) {
        ...
        {
            ch->pcdata->learned[sn] = 1;
            if (deduct) ch->pcdata->points += (int16_t)skill_table[sn].rating[arch];
        }
        return;
    }
    ...
    if (gn != -1) {
        if (ch->pcdata->group_known[gn] == false) {
            ch->pcdata->group_known[gn] = true;
            if (deduct) ch->pcdata->points += (int16_t)group_table[gn].rating[arch];
        }
...
int race_exp_per_level(int race, int class_, int points)
{
    ...
    Archetype arch = class_table[class_].arch;

    if (points < 40)
        return 1000 * (race_table[race].arch_mult[arch] ?
            race_table[race].arch_mult[arch] / 100 : 1);
 ...
 /* processing */
    ...
    return expl * race_table[race].arch_mult[arch] / 100;
}

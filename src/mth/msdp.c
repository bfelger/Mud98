...
char* msdp_get_var(Descriptor* d, char* var)
{
	int index;
...
void process_msdp_index_val(Descriptor* d, int var_index, char* val)
{
	int val_index;
...
void process_msdp_array(Descriptor* d, int var_index, char* val)
{
...
void msdp_command_list(Descriptor* d, int index)
{
	char buf[MAX_STRING_LENGTH] = { 0 };
	char* ptr = buf;
	int flag;
...
void msdp_command_report(Descriptor* d, int index)
{
...
void msdp_command_reset(Descriptor* d, int index)
{
	int flag;
...
void msdp_command_send(Descriptor* d, int index)
{
...
void msdp_command_unreport(Descriptor* d, int index)
{
	if (!HAS_BIT(msdp_table[index].flags, MSDP_FLAG_REPORTABLE))
	{
		return;
...
void msdp_configure_arachnos(Descriptor* d, int index)
{
	...
	char mud_name[MAX_INPUT_LENGTH] = { 0 };
	char mud_host[MAX_INPUT_LENGTH] = { 0 };
	char mud_port[MAX_INPUT_LENGTH] = { 0 };
	char msg_user[MAX_INPUT_LENGTH] = { 0 };
	char msg_time[MAX_INPUT_LENGTH] = { 0 };
	char msg_body[MAX_INPUT_LENGTH] = { 0 };
	char mud_players[MAX_INPUT_LENGTH] = { 0 };
	char mud_uptime[MAX_INPUT_LENGTH] = { 0 };
	char mud_update[MAX_INPUT_LENGTH] = { 0 };
	char* pti, * pto;
...
void msdp_configure_edit_buffer(Descriptor* d, int index)
{
...
//	{ "EDIT_BUFFER",            MSDP_FLAG_CONFIGURABLE|MSDP_FLAG_REPORTABLE,    msdp_configure_edit_buffer },
	...
	{ "IN_COMBAT",              MSDP_FLAG_SENDABLE | MSDP_FLAG_REPORTABLE,		NULL },
	...
	{ "OPPONENT_HEALTH",        MSDP_FLAG_SENDABLE | MSDP_FLAG_REPORTABLE,		NULL },
	{ "OPPONENT_HEALTH_MAX",    MSDP_FLAG_SENDABLE | MSDP_FLAG_REPORTABLE,		NULL },
	{ "OPPONENT_LEVEL",         MSDP_FLAG_SENDABLE | MSDP_FLAG_REPORTABLE,		NULL },
	{ "OPPONENT_NAME",          MSDP_FLAG_SENDABLE | MSDP_FLAG_REPORTABLE,		NULL },
	{ "POSITION",               MSDP_FLAG_SENDABLE | MSDP_FLAG_REPORTABLE,		NULL },
